...
//门诊药品
class ImportOutDrug implements BeforeImportProcess {
  static Logger logger = LoggerFactory.getLogger("org.ssh.pm.script.ImportOutDrug");

  void processImport(String startDate, String endDate) throws ImportException{
     println "it ok groovy";
     //logger.debug("门诊药品-processImport-{}-{}", new Object[]{startDate, endDate});
     logger.debug("门诊药品-processImport-{}-{}", new Object[]{startDate, endDate});
  ...
  void processOneDayImport(String oneDay) throws ImportException{
      logger.debug("门诊药品-processOneDayImport-"+oneDay);

      //r(oneDay);
   }

  void r(String startDate, String endDate) throws ImportException{
      ...
      logger.info("门诊药品-processOneDayImport-" + index + " records," + (System.currentTimeMillis() - start) + " ms");

      }catch(Exception dd){
          logger.error("门诊药品-processOneDayImport-" + dd);

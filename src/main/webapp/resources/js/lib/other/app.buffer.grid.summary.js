Ext.grid.BufferGridSummary = function(config){
    Ext.apply(this, config);
};

Ext.extend(Ext.grid.BufferGridSummary, Ext.util.Observable, {
    init : function(grid){
        this.grid = grid;
    ...
    renderSummary : function(o, cs){
      ...
      var tstyle = 'width:' + this.view.getTotalWidth() + ';';
      var  grpct = this.cellTpl, grprt = this.rowTpl;
    ...
    calculate : function(rs, cs){
        ...
        for(var j = 0, jlen = rs.length; j < jlen; j++){
            ...
            for(var i = 0, len = cs.length; i < len; i++){
                ...
                cf = cfg[i];
                if(cf.summaryType){
                    data[c.name] = Ext.grid.BufferGridSummary.Calculations[cf.summaryType](data[c.name] || 0, r, c.name, data);
                }
            }
        }
        return data;
...
Ext.grid.BufferGridSummary.Calculations = {
